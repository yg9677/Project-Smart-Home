...
public class ReceiveController {
    ...
    private void getRoomList(Order order){
        ...
        CommunicateTask AirCleanerTask = new AirCleanerTask();
        CommunicateTask SmartWindowTask = new WindowTask();
        CommunicateTask moodLightTask = new MoodLightTask();
        ...
        try {
            for (int i = 0; i < devicesList.size(); i++) {
                //현재 정보 불러오기
                if (devicesList.get(i).getName().equals("공기청정기")) {
                    ACLcurrent = AirCleanerTask.execute(MainActivity.myCode, devicesList.get(i).getModel(), "current").get();
                    System.out.println(ACLcurrent);
                    devicesList.get(i).setState(Integer.parseInt(ACLcurrent));
                } else if (devicesList.get(i).getName().equals("스마트창문")) {
                    windowCurrent = SmartWindowTask.execute(MainActivity.myCode, devicesList.get(i).getModel(), "current").get();
                    System.out.println(windowCurrent);
                    devicesList.get(i).setState(Integer.parseInt(windowCurrent));
                } else if (devicesList.get(i).getName().equals("무드등")) {
                    modeCurrent = moodLightTask.execute(MainActivity.myCode, devicesList.get(i).getModel(), "current").get();
                    System.out.println(modeCurrent);
                    devicesList.get(i).setState(Integer.parseInt(modeCurrent));
                }
            }
        } catch (InterruptedException e) {
            e.printStackTrace();
